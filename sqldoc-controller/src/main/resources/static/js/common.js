    ...
    jQuery.fn.extend({
        serializeObject: function (addBlank) {
            ...
            $.each(array, function (arrayIndex, dataObj) {
                if ((dataObj.value && $.trim(dataObj.value) != "") || addBlank) {
                    if (!obj.hasOwnProperty(dataObj.name)) {	// 如果不存在该属性，则添加该属性
                        obj[dataObj.name] = dataObj.value || null;

                    } else { // 如果存在该属性，则视为数组
                        if (obj[dataObj.name] != null && $.isArray(obj[dataObj.name]) == true) {
                            obj[dataObj.name].push(dataObj.value || null);
                        } else {
                            var newArray = [];
                            newArray.push(obj[dataObj.name]);
                            newArray.push(dataObj.value || null);
